...
@interface ZRQRCodeScanView3()<ZRQRCodeViewDelegate>
...
@implementation ZRQRCodeScanView3
...
- (void)openQRCodeScan:(UIViewController *)viewController
{
    ...
    ZRQRCodeViewController *qrCode = [[ZRQRCodeViewController alloc] initWithScanType:ZRQRCodeScanTypeContinuation customView:self navigationBarTitle:@"Scan Barcode"];
    qrCode.VCTintColor = [UIColor whiteColor];
    qrCode.qrcodeDelegate = self;
    qrCode.scanCodeKind = ZRScanCodeKindBarCode;
...
- (void)addSomeView:(CGRect)rect
{
    ...
    [bottomText setText:@"对准条形码，即可自动扫描"];
    ...
    UIView *scanFrame = [[UIView alloc] initWithFrame:CGRectMake(sfX, sfY, sfW, sfH)];
...
- (void)switchQRCodeOrBarCode:(UISwitch *)switchButton
{
    if ([switchButton isOn]) {
        ...
        [[[ZRQRCodeScanView2 alloc] init] openQRCodeScan:self.lastViewController];
    } else {
